...
def format_fixed_table(rows: List[Tuple[str, str, str]], widths: Tuple[int, int, int] = (6, 60, 80),
...
def build_gen_parser(ap: argparse.ArgumentParser) -> None:
    ...
    ap.add_argument("--audit-limit", type=int, default=40,
                    help="How many audit entries to print to the terminal (0 = all). Default: 40.")
    ap.add_argument("--audit-out", default=None,
                    help="Write the FULL audit to this file as TSV (crate\t table\t roll\t result).")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if not args.seed:
        # Use base62 by default to avoid shell-quoting headaches that ascii94 can cause
    ...
    for seq in sequences:
        ...
        # Write full audit to a file if requested
        if args.audit_out and state.audit:
            try:
                with open(args.audit_out, "w", encoding="utf-8", newline="") as f:
                    f.write("crate\ttable\troll\tresult\n")
                    for ev in state.audit:
                        # TSV-safe-ish: replace newlines and tabs in result
                        res = str(ev.result).replace("\t", " ").replace("\r", " ").replace("\n", " ")
                        f.write(f"{ev.origin}\t{ev.table}\t{ev.roll}\t{res}\n")
            except OSError as e:
                print(f"# Failed to write audit to {args.audit_out!r}: {e}", file=sys.stderr)

        if args.audit and state.audit:
            print("Audit (table rolls)")
            print("-------------------")
            limit = int(getattr(args, "audit_limit", 40))
            to_print = state.audit if limit == 0 else state.audit[-limit:]
            for ev in to_print:
                print(f"{ev.origin}\t{ev.table}\troll={ev.roll}\t{ev.result}")
            if limit != 0 and len(state.audit) > limit:
                print(f"... ({len(state.audit)} total audit entries; wrote full audit to {args.audit_out!r})" if args.audit_out else f"... ({len(state.audit)} total audit entries)")
            print()
